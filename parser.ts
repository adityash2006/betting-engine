...
const inningsOddEvenRule: BetParserRule = {
  ...
  transform: ([, innings, over, oddEven]) => ({
    context: {
      innings: Number(innings),
      over: Number(over),
    },
    condition: oddEven.toLowerCase(),
  }),
};
...
const playerStatRule: BetParserRule = {
  ...
  transform: ([, player, metricRaw]) => {
    const normalizedMetric = normalizePlayerMetric(metricRaw);
    const isOddEven = /odd|even/i.test(metricRaw);
    return {
      metric: normalizedMetric,
      condition: isOddEven ? metricRaw.toLowerCase().includes('odd') ? 'odd' : 'even' : undefined,
      context: {
...
const partnershipRule: BetParserRule = {
  ...
  transform: ([, wicket, metricRaw, team]) => {
...
const specialOddEvenRule: BetParserRule = {
  ...
  transform: ([, oddEven]) => ({
    condition: oddEven.toLowerCase() as 'odd' | 'even',
  }),
};
...
function normalizeTeam(value: string): string {
  return stripTrailingTokens(sanitizeLabel(value)).toUpperCase();
}
